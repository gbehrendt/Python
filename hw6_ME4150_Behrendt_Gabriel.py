...
# Decision boundary
print('\n')
print('Win vs non-Win Glass Logistic decision boundary \n ({:.2f})+({:.2f})(x1)+({:.2f})(x2)=0'
...
"""
The two features that I chose to try to predict the compressive strength classes were the amounts of blast furnace slag and 
fly ash. After using the prediction model the confusion matrix was created as shown in the console. The model was able to 
predict the correct class about 58% of the time. The model predicted the correct class at about the same rate for both positives 
and negatives. This particular model and test set resulted in 73 true negatives, 53 false negatives, 33 false positives, and 47 true positives.
In conclusion, the two features chosen, blast furnace slag and fly ash, are not reliable predictors of compressive strength in concrete.
